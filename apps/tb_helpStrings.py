...
counterAnimLayer = 'creates an additive layer which will counter the animation in the selected layer. '\
                    'Last selected object is the driver, other objects will be baked into an additive '\
...
toggleMotionTrail = 'Toggles motion trail states. Creates a motion trail if needed. If nothing is selected, '\
                     'all trails are toggled on/off'\
...
gravity = {'quickJump': 'Uses the highlighted time slider range to plot a ballistic arc between the start and end positions',
           'jumpAllKeyWindows': 'Uses pairs of keys to plot multiple jump arcs using the highlighted time slider range',
           'jumpUsingInitialFrameVelocity': 'Plots a ballistic arc for the duration of the highlighted time slider range using the first frames velocity'}
